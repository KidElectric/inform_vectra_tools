...
def check_tls_from_labeled_tls(df, 
                               labeled_tls,
                               tls_num,
                               cd20_min = 10,
                               n_comp = 10,
...
def plot_tls_df_coord(tls_df,tls_num,ax):
    xy = tls_df.loc[tls_num,'coords']
    ax.plot(xy[:,0],xy[:,1])
    
def celldf_to_tls_df(df,
                cd20_min= 10,
                n_comp = 10, #Number of gaussians to use in mixture model
                cd4_min_percent = 0,
                contour_thresh = 14,
...
def tls_df_to_long_cell_xy(tls_df,
                           use_cells):
    '''
